...
router.post("/:examid/:sid/", async (req, res) => {
  try {
    ...
    // if (!answer_doc) {
    //   answer_doc = await Answer.findOne({ eid: req.params.examid, sid: "0" });
    // }
    // console.log("answer_doc",answer_doc);
    // answer_doc.sid = req.params.sid;
...
// router.get("/:examid/:sid/getResult", isStudent, async (req, res) => {
//   try {
//     const answer_doc = await Answer.findOne({ eid: req.params.examid });
//     const { answers } = answer_doc;
//     const answers_len = answers.length;
//     const exam = await Exam.findById(req.params.examid);
//     let total_points = 0;
...
//     const exam = await Exam.findById(req.params.examid);
//     const question = exam.questions[q - 1];
//     const answer = await Answer.findOne({ eid: req.params.examid });
//     res.send({
//       data: question,
//       selectedOption: answer?.answers[q - 1]?.selectedOption,
//     });
...
//     for (var i = 0; i < answers.length; ++i) {
//       answers[i].correct_option =
//         questions[i].options[mapper[answers[i].correct_option]];
//       answers[i].selectedOption =
//         questions[i].options[mapper[answers[i].selectedOption]];
//     }
//     await answer_doc.save();
